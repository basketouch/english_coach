/* Basketball Coach English — the app shell + screens.
   Theme-aware: every instance reads CSS vars set on its root, so the same
   component renders all visual directions. Exports window.CoachApp. */
...
function VocabScreen({ speak }) {
  const cats = window.BBALL.categories;
...
function PhrasesScreen({ speak }) {
  const groups = window.BBALL.phraseGroups;
  return (
    <div className="bb-scroll">
      <header className="bb-top">
...
function DrillsScreen({ speak }) {
  const drills = window.BBALL.drills;
  const [open, setOpen] = useState(null);
...
function buildDeck() {
  const out = [];
  window.BBALL.categories.forEach(c =>
...
function CoachApp({ theme }) {
  ...
  const focusTerms = useMemo(() => {
    const off = window.BBALL.categories.find(c => c.id === 'offense').terms;
    const def = window.BBALL.categories.find(c => c.id === 'defense').terms;
    return [off[0], def[3], off[10]]; // pick & roll, close out, spacing
  }, []);
  const phraseOfDay = window.BBALL.phraseGroups[2].items[5]; // "Make it sharp."
